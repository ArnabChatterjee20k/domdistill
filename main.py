...
def get_best_chunks_only(i=0, chunks: list[str] | None = None, query="", heading="", penalty = 0.0001) -> tuple[float, list[str]]:
    if chunks is None:
        chunks = []
    ...
    def helper(i):
        ...
        for j in range(i, len(chunks)):
            ...
            current_chunk_score = get_score_for_chunk(query, heading, current_best_chunks_merged, penalty)
            ...
            if total_score_in_current_node > current_best[0]:
                current_best = (total_score_in_current_node, [current_best_chunks_merged, *rest_segment])
        return current_best

    return helper(i)

# returns best chunks merged in a way + orphan chunks as well
def get_chunks(i=0, chunks: list[str] | None = None, query="", heading="", penalty = 0.0001) -> tuple[float, list[str], list[str]]:
    if chunks is None:
        chunks = []
